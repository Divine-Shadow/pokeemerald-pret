...
static bool32 SharedPower_GetNextMoveEndAbility(u32 battler, u32 caseId, u16 *abilityOut);
static bool32 TryMoveEndAbilityEffect(u32 battler, u16 ability, u32 move, u32 moveType);
static bool32 TryMoveEndAttackerAbilityEffect(u32 battler, u16 ability, u32 move);
static bool32 TryMoveEndOtherAbilityEffect(u32 battler, u16 ability, u32 move);
...
static bool32 ShouldTeraShellDistortTypeMatchups(u32 move, u32 battlerDef)
{
    if (!gSpecialStatuses[battlerDef].distortedTypeMatchups
     ...
     && HasActiveAbility(battlerDef, ABILITY_TERA_SHELL))
        return TRUE;
...
bool32 HandleMoveTargetRedirection(void)
{
     ...
     && !IsBattlerAlly(gBattlerAttacker, gSideTimers[side].followmeTarget))
    {
        ...
        return TRUE;
           ...
           && ((!HasActiveAbility(gBattleStruct->moveTarget[gBattlerAttacker], ABILITY_LIGHTNING_ROD) && moveType == TYPE_ELECTRIC)
            || (!HasActiveAbility(gBattleStruct->moveTarget[gBattlerAttacker], ABILITY_STORM_DRAIN) && moveType == TYPE_WATER)))
    {
        ...
        for (battler = 0; battler < gBattlersCount; battler++)
        {
            if ((B_REDIRECT_ABILITY_ALLIES >= GEN_4 || !IsBattlerAlly(gBattlerAttacker, battler))
                ...
                && ((HasActiveAbility(battler, ABILITY_LIGHTNING_ROD) && moveType == TYPE_ELECTRIC)
                 || (HasActiveAbility(battler, ABILITY_STORM_DRAIN) && moveType == TYPE_WATER))
        ...
        if (redirectorOrderNum != MAX_BATTLERS_COUNT)
        {
            ...
            if (moveType == TYPE_ELECTRIC && gCurrentMove != MOVE_TEATIME)
            {
...
u32 TrySetCantSelectMoveBattleScript(u32 battler)
{
    ...
    if (DYNAMAX_BYPASS_CHECK && HasActiveAbility(battler, ABILITY_GORILLA_TACTICS) && *choicedMove != MOVE_NONE
...
u32 CheckMoveLimitations(u32 battler, u8 unusableMoves, u16 check)
{
    ...
    for (i = 0; i < MAX_MON_MOVES; i++)
    {
        ...
        // Stuff Cheeks
            ...
            unusableMoves |= 1u << i;
        // Gorilla Tactics
        else if (check & MOVE_LIMITATION_CHOICE_ITEM && HasActiveAbility(battler, ABILITY_GORILLA_TACTICS) && *choicedMove != MOVE_NONE && *choicedMove != MOVE_UNAVAILABLE && *choicedMove != move)
            unusableMoves |= 1u << i;
...
static bool32 TryRecordActiveRipen(u32 battler)
{
    if (!IsAbilityAndRecord(battler, HasActiveAbility(battler, ABILITY_RIPEN) ? ABILITY_RIPEN : ABILITY_NONE, ABILITY_RIPEN))
    {
        gLastUsedAbility = ABILITY_NONE;
        return FALSE;
    }

    gLastUsedAbility = ABILITY_RIPEN;
...
static enum MoveCanceller CancellerAsleepOrFrozen(void)
{
    if (gBattleMons[gBattlerAttacker].status1 & STATUS1_SLEEP)
    {
        ...
        else
        {
            ...
            {
                RecordAbilityBattle(gBattlerAttacker, ABILITY_EARLY_BIRD);
                toSub = 2;
            }
            else
            {
...
static enum MoveCanceller CancellerChoiceLock(void)
{
    ...
    if (gChosenMove != MOVE_STRUGGLE
     ...
     && (IsHoldEffectChoice(holdEffect) || HasActiveAbility(gBattlerAttacker, ABILITY_GORILLA_TACTICS)))
...
static enum MoveCanceller CancellerPowderStatus(void)
{
    if (TryActivatePowderStatus(gCurrentMove))
    {
        ...
        if (!HasActiveAbility(gBattlerAttacker, ABILITY_MAGIC_GUARD))
...
static enum MoveCanceller CancellerMultiTargetMoves(void)
{
    ...
    if (IsSpreadMove(moveTarget))
    {
        for (u32 battlerDef = 0; battlerDef < gBattlersCount; battlerDef++)
        {
            ...
            else if (GetBattlerMoveAbsorbingAbility(gBattlerAttacker, battlerDef, abilityDef, gCurrentMove, GetBattleMoveType(gCurrentMove)) != ABILITY_NONE)
            {
                gBattleStruct->moveResultFlags[battlerDef] = 0;
...
static bool32 IsPriorityMoveBlockingAbility(u32 ability)
{
    switch (ability)
    {
...
static bool32 CanSelectedAbilityBlockMove(u32 battlerAtk, u32 battlerDef, u32 ability, u32 move, s32 atkPriority, const u8 **battleScriptBlocksMove)
{
    switch (ability)
    {
    ...
    return *battleScriptBlocksMove != NULL;
}
...
bool32 CanAbilityBlockMove(u32 battlerAtk, u32 battlerDef, u32 abilityAtk, u32 abilityDef, u32 move, enum FunctionCallOption option)
{
    ...
    u32 blockingAbility = abilityDef;
    s32 atkPriority = 0;
    ...
    CanSelectedAbilityBlockMove(battlerAtk, battlerDef, abilityDef, move, atkPriority, &battleScriptBlocksMove);
    ...
    if (atkPriority > 0)
    {
         ...
         && !IsBattlerAlly(battlerAtk, partnerDef))
        {
            ...
            if (option == AI_CHECK)
                partnerAbility = gAiLogicData->abilities[partnerDef];
            else
                partnerAbility = GetBattlerAbility(partnerDef);

            if (IsPriorityMoveBlockingAbility(partnerAbility)
             && CanSelectedAbilityBlockMove(battlerAtk, battlerDef, partnerAbility, move, atkPriority, &battleScriptBlocksMove))
            {
                battlerAbility = partnerDef;
                blockingAbility = partnerAbility;
            }
            else if (option != AI_CHECK && SharedPower_IsEnabled())
            {
                for (u32 i = 0; i < ARRAY_COUNT(sBlockAbilities); i++)
                {
                    u32 ability = sBlockAbilities[i];

                    if (!IsPriorityMoveBlockingAbility(ability) || !HasActiveAbility(partnerDef, ability))
                        continue;

                    if (CanSelectedAbilityBlockMove(battlerAtk, battlerDef, ability, move, atkPriority, &battleScriptBlocksMove))
                    {
                        battlerAbility = partnerDef;
                        blockingAbility = ability;
                        break;
                    }
                }
            }
    ...
    if (battleScriptBlocksMove == NULL && option != AI_CHECK && SharedPower_IsEnabled())
    {
        for (u32 i = 0; i < ARRAY_COUNT(sBlockAbilities); i++)
        {
            u32 ability = sBlockAbilities[i];

            if (!HasActiveAbility(battlerDef, ability))
                continue;
    ...
    if (option == RUN_SCRIPT)
    {
        ...
        RecordAbilityBattle(battlerAbility, blockingAbility);
...
static bool32 SharedPower_GetNextMoveEndAbility(u32 battler, u32 caseId, u16 *abilityOut)
{
    ...
    if (gBattleStruct->sharedPowerMoveEndCaseId[battler] != caseId)
    {
        gBattleStruct->sharedPowerMoveEndCaseId[battler] = caseId;
        gBattleStruct->sharedPowerMoveEndIndex[battler] = 0;
    }

    index = gBattleStruct->sharedPowerMoveEndIndex[battler];
    ...
    while (TRUE)
    {
        if (index == 0)
        {
            ...
            {
                gBattleStruct->sharedPowerMoveEndIndex[battler] = index;
...
static bool32 TryMoveEndAbilityEffect(u32 battler, u16 ability, u32 move, u32 moveType)
{
    ...
    case ABILITY_MUMMY:
        if (IsBattlerAlive(gBattlerAttacker)
         ...
         && gDisableStructs[gBattlerAttacker].overwrittenAbility != ability
         ...
         && !gAbilitiesInfo[gBattleMons[gBattlerAttacker].ability].cantBeSuppressed)
        {
            ...
            gBattleMons[gBattlerAttacker].ability = gDisableStructs[gBattlerAttacker].overwrittenAbility = ability;
    ...
    case ABILITY_TANGLING_HAIR:
        if (IsBattlerAlive(gBattlerAttacker)
         && (CompareStat(gBattlerAttacker, STAT_SPEED, MIN_STAT_STAGE, CMP_GREATER_THAN) || HasActiveAbility(gBattlerAttacker, ABILITY_MIRROR_ARMOR))
    ...
    case ABILITY_CUTE_CHARM:
        if (IsBattlerAlive(gBattlerAttacker)
         ...
         && !IsAbilityAndRecord(gBattlerAttacker, HasActiveAbility(gBattlerAttacker, ABILITY_OBLIVIOUS) ? ABILITY_OBLIVIOUS : ABILITY_NONE, ABILITY_OBLIVIOUS)
         && !CanBattlerAvoidContactEffects(gBattlerAttacker, gBattlerTarget, GetBattlerAbility(gBattlerAttacker), GetBattlerHoldEffect(gBattlerAttacker, TRUE), move)
         && !IsAbilityOnSide(gBattlerAttacker, ABILITY_AROMA_VEIL))
        {
...
u32 AbilityBattleEffects(u32 caseID, u32 battler, u32 ability, u32 special, u32 moveArg)
{
    ...
    case ABILITYEFFECT_MOVE_END: // Think contact abilities.
        ...
        {
            u16 abilityIter;
            while (SharedPower_GetNextMoveEndAbility(battler, caseID, &abilityIter))
            {
                gLastUsedAbility = abilityIter;
                if (TryMoveEndAbilityEffect(battler, abilityIter, move, moveType))
                {
                    effect++;
                    break;
                }
        ...
        else if (TryMoveEndAbilityEffect(battler, gLastUsedAbility, move, moveType))
        {
            effect++;
        }
        break;
    case ABILITYEFFECT_MOVE_END_ATTACKER: // Same as above, but for attacker
        if (SharedPower_IsEnabled())
        {
            ...
            {
                gLastUsedAbility = abilityIter;
                if (TryMoveEndAttackerAbilityEffect(battler, abilityIter, move))
                {
                    effect++;
                    break;
                }
        ...
        else if (TryMoveEndAttackerAbilityEffect(battler, gLastUsedAbility, move))
        {
            effect++;
        }
        break;
    case ABILITYEFFECT_MOVE_END_OTHER: // Abilities that activate on *another* battler's moveend: Dancer, Soul-Heart, Receiver, Symbiosis
        if (SharedPower_IsEnabled())
        {
            ...
            {
                gLastUsedAbility = abilityIter;
                if (TryMoveEndOtherAbilityEffect(battler, abilityIter, move))
                {
                    effect++;
                    break;
                }
        ...
        else if (TryMoveEndOtherAbilityEffect(battler, GetBattlerAbility(battler), move))
        {
            effect++;
        }
        break;
...
{
    if (IsBattlerAlive(battler) && HasActiveAbility(battler, ability))
        return battler + 1;
    else if (IsBattlerAlive(BATTLE_PARTNER(battler)) && HasActiveAbility(BATTLE_PARTNER(battler), ability))
...
u32 IsAbilityOnField(u32 ability)
{
    ...
    for (i = 0; i < gBattlersCount; i++)
    {
        if (IsBattlerAlive(i) && HasActiveAbility(i, ability))
            return i + 1;
    }
...
u32 IsAbilityOnFieldExcept(u32 battler, u32 ability)
{
    ...
    for (i = 0; i < gBattlersCount; i++)
    {
        if (i != battler && IsBattlerAlive(i) && HasActiveAbility(i, ability))
            return i + 1;
    }
...
u32 GetBattlerMoveAbsorbingAbility(u32 battlerAtk, u32 battlerDef, u32 abilityDef, u32 move, u32 moveType)
{
    if (CanAbilityAbsorbMove(battlerAtk, battlerDef, abilityDef, move, moveType, CHECK_TRIGGER))
        return abilityDef;

    if (abilityDef != ABILITY_LIGHTNING_ROD
     && HasActiveAbility(battlerDef, ABILITY_LIGHTNING_ROD)
     && CanAbilityAbsorbMove(battlerAtk, battlerDef, ABILITY_LIGHTNING_ROD, move, moveType, CHECK_TRIGGER))
        return ABILITY_LIGHTNING_ROD;

    if (abilityDef != ABILITY_STORM_DRAIN
     && HasActiveAbility(battlerDef, ABILITY_STORM_DRAIN)
     && CanAbilityAbsorbMove(battlerAtk, battlerDef, ABILITY_STORM_DRAIN, move, moveType, CHECK_TRIGGER))
...
bool32 CanBeConfused(u32 battler)
{
    if (gBattleMons[battler].volatiles.confusionTurns > 0
     ...
     || IsAbilityAndRecord(battler, HasActiveAbility(battler, ABILITY_OWN_TEMPO) ? ABILITY_OWN_TEMPO : ABILITY_NONE, ABILITY_OWN_TEMPO))
        return FALSE;
    return TRUE;
}
...
bool32 HasEnoughHpToEatBerry(u32 battler, u32 hpFraction, u32 itemId)
{
    ...
    if (hpFraction <= 4 && isBerry
         ...
         && IsAbilityAndRecord(battler, HasActiveAbility(battler, ABILITY_GLUTTONY) ? ABILITY_GLUTTONY : ABILITY_NONE, ABILITY_GLUTTONY))
        return TRUE;
...
static enum ItemEffect HealConfuseBerry(u32 battler, u32 itemId, u32 flavorId, enum ItemCaseId caseID)
{
     ...
     && (B_HEAL_BLOCKING < GEN_5 || !gBattleMons[battler].volatiles.healBlock))
    {
        ...
        if (TryRecordActiveRipen(battler))
        {
...
static u32 ItemHealHp(u32 battler, u32 itemId, enum ItemCaseId caseID, bool32 percentHeal)
{
        ...
        && HasEnoughHpToEatBerry(battler, 2, itemId))
    {
        ...
        if (GetItemPocket(itemId) == POCKET_BERRIES
         && TryRecordActiveRipen(battler))
            gBattleStruct->moveDamage[battler] *= 2;

        // In SWSH, Berry Juice shows the ability pop-up but has no effect. This is mimicked here.
        if (GetItemPocket(itemId) != POCKET_BERRIES && HasActiveAbility(battler, ABILITY_RIPEN))
            TryRecordActiveRipen(battler);
        else if (gLastUsedAbility != ABILITY_RIPEN)
            gBattlerAbility = battler;
        if (caseID == ITEMEFFECT_ON_SWITCH_IN_FIRST_TURN || caseID == ITEMEFFECT_NORMAL)
            BattleScriptExecute(BattleScript_ItemHealHP_RemoveItemEnd2);
        else
...
u32 ItemBattleEffects(enum ItemCaseId caseID, u32 battler)
{
    ...
    case ITEMEFFECT_TRY_HEALING:
        if (gBattleMons[battler].hp)
        {
            ...
            case HOLD_EFFECT_BLACK_SLUDGE:
                ...
                else if (!HasActiveAbility(battler, ABILITY_MAGIC_GUARD))
                {
                    gBattleStruct->moveDamage[battler] = GetNonDynamaxMaxHP(battler) / 8;
                    if (gBattleStruct->moveDamage[battler] == 0)
                        gBattleStruct->moveDamage[battler] = 1;
    ...
    case ITEMEFFECT_TARGET:
        if (!(gBattleStruct->moveResultFlags[gBattlerTarget] & MOVE_RESULT_NO_EFFECT))
        {
            ...
            case HOLD_EFFECT_CURE_PSN:
                if (gBattleMons[battler].status1 & STATUS1_PSN_ANY && !UnnerveOn(battler, gLastUsedItem) && HasActiveAbility(gBattlerAttacker, ABILITY_TOXIC_CHAIN) && GetMoveEffect(gCurrentMove) == EFFECT_KNOCK_OFF)
                {
                    gBattleScripting.battler = battler;
                    gBattleMons[battler].status1 &= ~(STATUS1_PSN_ANY | STATUS1_TOXIC_COUNTER);
                    BattleScriptExecute(BattleScript_BerryCurePsnEnd2);
...
u32 GetBattleMoveTarget(u16 move, u8 setTarget)
{
    ...
    case MOVE_TARGET_OPPONENT:
        ...
        else
        {
            ...
            if (moveType == TYPE_ELECTRIC && !HasActiveAbility(targetBattler, ABILITY_LIGHTNING_ROD))
            {
                ...
                if (battlerAbilityOnField > 0 && (battlerAbilityOnField - 1) != gBattlerAttacker)
                {
                    ...
                    RecordAbilityBattle(targetBattler, ABILITY_LIGHTNING_ROD);
                    gSpecialStatuses[targetBattler].lightningRodRedirected = TRUE;
                }
            }
            else if (moveType == TYPE_WATER && !HasActiveAbility(targetBattler, ABILITY_STORM_DRAIN))
            {
                if (B_REDIRECT_ABILITY_ALLIES >= GEN_4)
                ...
                {
                    targetBattler = battlerAbilityOnField - 1;
                    RecordAbilityBattle(targetBattler, ABILITY_STORM_DRAIN);
...
enum ItemHoldEffect GetBattlerHoldEffectInternal(u32 battler, bool32 checkNegating, bool32 checkAbility)
{
    if (checkNegating)
    {
        ...
        if (gFieldStatuses & STATUS_FIELD_MAGIC_ROOM)
            return HOLD_EFFECT_NONE;
        if (checkAbility && HasActiveAbility(battler, ABILITY_KLUTZ))
            return HOLD_EFFECT_NONE;
    }
...
bool32 IsBattlerProtected(u32 battlerAtk, u32 battlerDef, u32 move)
{
    ...
    if (gProtectStructs[battlerDef].protected != PROTECT_MAX_GUARD && !MoveIgnoresProtect(move))
    {
        ...
        if (HasActiveAbility(battlerAtk, ABILITY_UNSEEN_FIST)
...
static bool32 IsBattlerGroundedInverseCheck(u32 battler, enum InverseBattleCheck checkInverse, enum IronBallCheck checkIronBall)
{
    ...
    if (holdEffect == HOLD_EFFECT_AIR_BALLOON)
        return FALSE;
    if (gAiLogicData->aiCalcInProgress ? gAiLogicData->abilities[battler] == ABILITY_LEVITATE : HasActiveAbility(battler, ABILITY_LEVITATE))
    {
        if (!gAiLogicData->aiCalcInProgress)
            RecordAbilityBattle(battler, ABILITY_LEVITATE);
        return FALSE;
    }
...
static inline u32 CalcMoveBasePower(struct DamageContext *ctx)
{
    ...
    case EFFECT_DOUBLE_POWER_ON_ARG_STATUS:
        ...
        if ((gBattleMons[battlerDef].status1 | (STATUS1_SLEEP * HasActiveAbility(battlerDef, ABILITY_COMATOSE))) & GetMoveEffectArg_Status(move)
...
static bool32 ApplyAttackerBasePowerAbility(u16 ability)
{
    ...
    case ABILITY_TECHNICIAN:
        if (ctx->basePower <= 60)
           *ctx->modifier = uq4_12_multiply(*ctx->modifier, UQ_4_12(1.5));
        break;
    case ABILITY_FLARE_BOOST:
        if (gBattleMons[ctx->battlerAtk].status1 & STATUS1_BURN && IsBattleMoveSpecial(ctx->move))
           *ctx->modifier = uq4_12_multiply(*ctx->modifier, UQ_4_12(1.5));
        break;
    case ABILITY_TOXIC_BOOST:
        if (gBattleMons[ctx->battlerAtk].status1 & STATUS1_PSN_ANY && IsBattleMovePhysical(ctx->move))
           *ctx->modifier = uq4_12_multiply(*ctx->modifier, UQ_4_12(1.5));
        break;
    case ABILITY_RECKLESS:
        if (ctx->moveEffect == EFFECT_RECOIL || ctx->moveEffect == EFFECT_RECOIL_IF_MISS)
           *ctx->modifier = uq4_12_multiply(*ctx->modifier, UQ_4_12(1.2));
        break;
    ...
    case ABILITY_SHEER_FORCE:
        if (MoveIsAffectedBySheerForce(ctx->move))
           *ctx->modifier = uq4_12_multiply(*ctx->modifier, UQ_4_12(1.3));
        break;
    case ABILITY_SAND_FORCE:
        if ((ctx->moveType == TYPE_STEEL || ctx->moveType == TYPE_ROCK || ctx->moveType == TYPE_GROUND)
            && ctx->ctx->weather & B_WEATHER_SANDSTORM)
           *ctx->modifier = uq4_12_multiply(*ctx->modifier, UQ_4_12(1.3));
        break;
    case ABILITY_RIVALRY:
        if (AreBattlersOfSameGender(ctx->battlerAtk, ctx->battlerDef))
            *ctx->modifier = uq4_12_multiply(*ctx->modifier, UQ_4_12(1.25));
        else if (AreBattlersOfOppositeGender(ctx->battlerAtk, ctx->battlerDef))
            *ctx->modifier = uq4_12_multiply(*ctx->modifier, UQ_4_12(0.75));
        break;
    case ABILITY_ANALYTIC:
        if (GetBattlerTurnOrderNum(ctx->battlerAtk) == gBattlersCount - 1 && ctx->move != MOVE_FUTURE_SIGHT && ctx->move != MOVE_DOOM_DESIRE)
           *ctx->modifier = uq4_12_multiply(*ctx->modifier, UQ_4_12(1.3));
        break;
    case ABILITY_TOUGH_CLAWS:
        if (IsMoveMakingContact(ctx->battlerAtk, ctx->battlerDef, ctx->ctx->abilityAtk, ctx->ctx->holdEffectAtk, ctx->ctx->move))
           *ctx->modifier = uq4_12_multiply(*ctx->modifier, UQ_4_12(1.3));
        break;
    case ABILITY_STRONG_JAW:
        if (IsBitingMove(ctx->move))
           *ctx->modifier = uq4_12_multiply(*ctx->modifier, UQ_4_12(1.5));
        break;
    case ABILITY_MEGA_LAUNCHER:
        if (IsPulseMove(ctx->move))
           *ctx->modifier = uq4_12_multiply(*ctx->modifier, UQ_4_12(1.5));
        break;
    case ABILITY_WATER_BUBBLE:
        if (ctx->moveType == TYPE_WATER)
           *ctx->modifier = uq4_12_multiply(*ctx->modifier, UQ_4_12(2.0));
        break;
    ...
    case ABILITY_PIXILATE:
        if (ctx->moveType == TYPE_FAIRY && gBattleStruct->battlerState[ctx->battlerAtk].ateBoost)
            *ctx->modifier = uq4_12_multiply(*ctx->modifier, UQ_4_12(GetGenConfig(GEN_CONFIG_ATE_MULTIPLIER) >= GEN_7 ? 1.2 : 1.3));
        break;
    case ABILITY_GALVANIZE:
        if (ctx->moveType == TYPE_ELECTRIC && gBattleStruct->battlerState[ctx->battlerAtk].ateBoost)
            *ctx->modifier = uq4_12_multiply(*ctx->modifier, UQ_4_12(GetGenConfig(GEN_CONFIG_ATE_MULTIPLIER) >= GEN_7 ? 1.2 : 1.3));
        break;
    case ABILITY_REFRIGERATE:
        if (ctx->moveType == TYPE_ICE && gBattleStruct->battlerState[ctx->battlerAtk].ateBoost)
            *ctx->modifier = uq4_12_multiply(*ctx->modifier, UQ_4_12(GetGenConfig(GEN_CONFIG_ATE_MULTIPLIER) >= GEN_7 ? 1.2 : 1.3));
        break;
    case ABILITY_AERILATE:
        if (ctx->moveType == TYPE_FLYING && gBattleStruct->battlerState[ctx->battlerAtk].ateBoost)
            *ctx->modifier = uq4_12_multiply(*ctx->modifier, UQ_4_12(GetGenConfig(GEN_CONFIG_ATE_MULTIPLIER) >= GEN_7 ? 1.2 : 1.3));
        break;
    case ABILITY_NORMALIZE:
        if (ctx->moveType == TYPE_NORMAL && gBattleStruct->battlerState[ctx->battlerAtk].ateBoost && GetGenConfig(GEN_CONFIG_ATE_MULTIPLIER) >= GEN_7)
            *ctx->modifier = uq4_12_multiply(*ctx->modifier, UQ_4_12(1.2));
        break;
    ...
    case ABILITY_SHARPNESS:
        ...
        break;
    case ABILITY_SUPREME_OVERLORD:
        *ctx->modifier = uq4_12_multiply(*ctx->modifier, GetSupremeOverlordModifier(ctx->battlerAtk));
        break;
    }
...
static bool32 ApplyDefenderBasePowerAbility(u16 ability)
{
    struct BasePowerAbilityCtx *ctx = sBasePowerAbilityCtx;
    ...
    case ABILITY_PROTOSYNTHESIS:
        {
            u8 defHighestStat = GetHighestStatId(ctx->battlerDef);
            if (((ctx->ctx->weather & B_WEATHER_SUN && HasWeatherEffect()) || gDisableStructs[ctx->battlerDef].boosterEnergyActivated)
             && ((IsBattleMovePhysical(ctx->move) && defHighestStat == STAT_DEF) || (IsBattleMoveSpecial(ctx->move) && defHighestStat == STAT_SPDEF))
             && !(gBattleMons[ctx->battlerDef].volatiles.transformed))
                *ctx->modifier = uq4_12_multiply(*ctx->modifier, UQ_4_12(0.7));
        }
        break;
    case ABILITY_QUARK_DRIVE:
        {
            u8 defHighestStat = GetHighestStatId(ctx->battlerDef);
            if ((gFieldStatuses & STATUS_FIELD_ELECTRIC_TERRAIN || gDisableStructs[ctx->battlerDef].boosterEnergyActivated)
             && ((IsBattleMovePhysical(ctx->move) && defHighestStat == STAT_DEF) || (IsBattleMoveSpecial(ctx->move) && defHighestStat == STAT_SPDEF))
             && !(gBattleMons[ctx->battlerDef].volatiles.transformed))
                *ctx->modifier = uq4_12_multiply(*ctx->modifier, UQ_4_12(0.7));
        }
...
static inline u32 CalcMoveBasePowerAfterModifiers(struct DamageContext *ctx)
{
    ...
    {
        struct BasePowerAbilityCtx abilityCtx = {
            .ctx = ctx,
            .modifier = &modifier,
            .basePower = basePower,
    ...
    {
        u32 partnerAtk = BATTLE_PARTNER(battlerAtk);

        if (HasActiveAbility(partnerAtk, ABILITY_BATTERY) && IsBattleMoveSpecial(move))
            modifier = uq4_12_multiply(modifier, UQ_4_12(1.3));
        if (HasActiveAbility(partnerAtk, ABILITY_POWER_SPOT))
            modifier = uq4_12_multiply(modifier, UQ_4_12(1.3));
        if (HasActiveAbility(partnerAtk, ABILITY_STEELY_SPIRIT) && moveType == TYPE_STEEL)
            modifier = uq4_12_multiply(modifier, UQ_4_12(1.5));
    }
...
static struct AttackStatAbilityCtx *sAttackStatAbilityCtx;

static bool32 ApplyAttackerAttackStatAbility(u16 ability)
{
    struct AttackStatAbilityCtx *ctx = sAttackStatAbilityCtx;
    ...
    case ABILITY_PURE_POWER:
        ...
        break;
    case ABILITY_SLOW_START:
        if (gDisableStructs[ctx->battlerAtk].slowStartTimer > gBattleTurnCounter)
            *ctx->modifier = uq4_12_multiply_half_down(*ctx->modifier, UQ_4_12(0.5));
        break;
    case ABILITY_SOLAR_POWER:
        if (IsBattleMoveSpecial(ctx->move) && IsBattlerWeatherAffected(ctx->battlerAtk, B_WEATHER_SUN))
            *ctx->modifier = uq4_12_multiply_half_down(*ctx->modifier, UQ_4_12(1.5));
        break;
    case ABILITY_DEFEATIST:
        if (gBattleMons[ctx->battlerAtk].hp <= (gBattleMons[ctx->battlerAtk].maxHP / 2))
            *ctx->modifier = uq4_12_multiply_half_down(*ctx->modifier, UQ_4_12(0.5));
        break;
    case ABILITY_FLASH_FIRE:
        if (ctx->moveType == TYPE_FIRE && gDisableStructs[ctx->battlerAtk].flashFireBoosted)
            *ctx->modifier = uq4_12_multiply_half_down(*ctx->modifier, UQ_4_12(1.5));
        break;
    case ABILITY_SWARM:
        if (ctx->moveType == TYPE_BUG && gBattleMons[ctx->battlerAtk].hp <= (gBattleMons[ctx->battlerAtk].maxHP / 3))
            *ctx->modifier = uq4_12_multiply_half_down(*ctx->modifier, UQ_4_12(1.5));
        break;
    case ABILITY_TORRENT:
        if (ctx->moveType == TYPE_WATER && gBattleMons[ctx->battlerAtk].hp <= (gBattleMons[ctx->battlerAtk].maxHP / 3))
            *ctx->modifier = uq4_12_multiply_half_down(*ctx->modifier, UQ_4_12(1.5));
        break;
    case ABILITY_BLAZE:
        if (ctx->moveType == TYPE_FIRE && gBattleMons[ctx->battlerAtk].hp <= (gBattleMons[ctx->battlerAtk].maxHP / 3))
            *ctx->modifier = uq4_12_multiply_half_down(*ctx->modifier, UQ_4_12(1.5));
        break;
    case ABILITY_OVERGROW:
        if (ctx->moveType == TYPE_GRASS && gBattleMons[ctx->battlerAtk].hp <= (gBattleMons[ctx->battlerAtk].maxHP / 3))
            *ctx->modifier = uq4_12_multiply_half_down(*ctx->modifier, UQ_4_12(1.5));
        break;
    case ABILITY_PLUS:
        if (IsBattleMoveSpecial(ctx->move) && IsBattlerAlive(BATTLE_PARTNER(ctx->battlerAtk)))
        {
            u32 partner = BATTLE_PARTNER(ctx->battlerAtk);
            if (HasActiveAbility(partner, ABILITY_MINUS)
             || (B_PLUS_MINUS_INTERACTION >= GEN_5 && HasActiveAbility(partner, ABILITY_PLUS)))
                *ctx->modifier = uq4_12_multiply_half_down(*ctx->modifier, UQ_4_12(1.5));
        }
        break;
    case ABILITY_MINUS:
        ...
        {
            u32 partner = BATTLE_PARTNER(ctx->battlerAtk);
            if (HasActiveAbility(partner, ABILITY_PLUS)
             || (B_PLUS_MINUS_INTERACTION >= GEN_5 && HasActiveAbility(partner, ABILITY_MINUS)))
                *ctx->modifier = uq4_12_multiply_half_down(*ctx->modifier, UQ_4_12(1.5));
        }
        break;
    case ABILITY_FLOWER_GIFT:
        if (gBattleMons[ctx->battlerAtk].species == SPECIES_CHERRIM_SUNSHINE
         && IsBattlerWeatherAffected(ctx->battlerAtk, B_WEATHER_SUN)
    ...
    case ABILITY_STAKEOUT:
        if (gDisableStructs[ctx->battlerDef].isFirstTurn == 2)
            *ctx->modifier = uq4_12_multiply_half_down(*ctx->modifier, UQ_4_12(2.0));
        break;
    case ABILITY_GUTS:
        if (gBattleMons[ctx->battlerAtk].status1 & STATUS1_ANY && IsBattleMovePhysical(ctx->move))
            *ctx->modifier = uq4_12_multiply_half_down(*ctx->modifier, UQ_4_12(1.5));
        break;
    ...
    case ABILITY_ROCKY_PAYLOAD:
        ...
        break;
    case ABILITY_PROTOSYNTHESIS:
        if (!(gBattleMons[ctx->battlerAtk].volatiles.transformed))
        {
            u32 atkHighestStat = GetHighestStatId(ctx->battlerAtk);
            if (((ctx->ctx->weather & B_WEATHER_SUN) && HasWeatherEffect()) || gDisableStructs[ctx->battlerAtk].boosterEnergyActivated)
            {
                if ((IsBattleMovePhysical(ctx->move) && atkHighestStat == STAT_ATK) || (IsBattleMoveSpecial(ctx->move) && atkHighestStat == STAT_SPATK))
                    *ctx->modifier = uq4_12_multiply(*ctx->modifier, UQ_4_12(1.3));
            }
        }
        break;
    case ABILITY_QUARK_DRIVE:
        ...
        {
            u32 atkHighestStat = GetHighestStatId(ctx->battlerAtk);
            if (gFieldStatuses & STATUS_FIELD_ELECTRIC_TERRAIN || gDisableStructs[ctx->battlerAtk].boosterEnergyActivated)
            {
                if ((IsBattleMovePhysical(ctx->move) && atkHighestStat == STAT_ATK) || (IsBattleMoveSpecial(ctx->move) && atkHighestStat == STAT_SPATK))
                    *ctx->modifier = uq4_12_multiply(*ctx->modifier, UQ_4_12(1.3));
            }
        ...
        break;
    case ABILITY_ORICHALCUM_PULSE:
        if ((ctx->ctx->weather & B_WEATHER_SUN) && HasWeatherEffect() && IsBattleMovePhysical(ctx->move))
           *ctx->modifier = uq4_12_multiply(*ctx->modifier, UQ_4_12(1.3333));
        break;
    case ABILITY_HADRON_ENGINE:
        if (gFieldStatuses & STATUS_FIELD_ELECTRIC_TERRAIN && IsBattleMoveSpecial(ctx->move))
           *ctx->modifier = uq4_12_multiply(*ctx->modifier, UQ_4_12(1.3333));
        break;
    }
...
static bool32 ApplyDefenderAttackStatAbility(u16 ability)
{
    struct AttackStatAbilityCtx *ctx = sAttackStatAbilityCtx;
...
static inline u32 CalcAttackStat(struct DamageContext *ctx)
{
    ...
    if (HasActiveAbility(battlerDef, ABILITY_UNAWARE))
        atkStage = DEFAULT_STAT_STAGE;
    ...
    // attacker's abilities
    {
        struct AttackStatAbilityCtx abilityCtx = {
            .ctx = ctx,
            .modifier = &modifier,
            .battlerAtk = battlerAtk,
            .battlerDef = battlerDef,
    ...
    if (IsBattlerAlive(BATTLE_PARTNER(battlerAtk)))
    {
        ...
        if (HasActiveAbility(partnerAtk, ABILITY_FLOWER_GIFT)
         && gBattleMons[partnerAtk].species == SPECIES_CHERRIM_SUNSHINE
         && IsBattlerWeatherAffected(partnerAtk, B_WEATHER_SUN)
         && IsBattleMovePhysical(move))
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(1.5));
    }

    // field abilities
    if (IsAbilityOnField(ABILITY_VESSEL_OF_RUIN) && !HasActiveAbility(battlerAtk, ABILITY_VESSEL_OF_RUIN) && IsBattleMoveSpecial(move))
        modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(0.75));

    if (IsAbilityOnField(ABILITY_TABLETS_OF_RUIN) && !HasActiveAbility(battlerAtk, ABILITY_TABLETS_OF_RUIN) && IsBattleMovePhysical(move))
        modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(0.75));
...
static struct DefenseStatAbilityCtx *sDefenseStatAbilityCtx;

static bool32 ApplyDefenderDefenseStatAbility(u16 ability)
{
    struct DefenseStatAbilityCtx *ctx = sDefenseStatAbilityCtx;
    ...
    case ABILITY_GRASS_PELT:
        ...
        break;
    case ABILITY_FLOWER_GIFT:
        if (gBattleMons[ctx->battlerDef].species == SPECIES_CHERRIM_SUNSHINE && IsBattlerWeatherAffected(ctx->battlerDef, B_WEATHER_SUN) && !ctx->usesDefStat)
            *ctx->modifier = uq4_12_multiply_half_down(*ctx->modifier, UQ_4_12(1.5));
        break;
    }
...
static inline u32 CalcDefenseStat(struct DamageContext *ctx)
{
    ...
    if (HasActiveAbility(ctx->battlerAtk, ABILITY_UNAWARE))
        defStage = DEFAULT_STAT_STAGE;
    ...
    // target's abilities
    {
        struct DefenseStatAbilityCtx abilityCtx = {
            .ctx = ctx,
            .modifier = &modifier,
            .battlerDef = battlerDef,
            .move = move,
    ...
    if (IsBattlerAlive(BATTLE_PARTNER(battlerDef)))
    {
        ...
        if (HasActiveAbility(partnerDef, ABILITY_FLOWER_GIFT)
         && gBattleMons[partnerDef].species == SPECIES_CHERRIM_SUNSHINE
         && IsBattlerWeatherAffected(partnerDef, B_WEATHER_SUN)
         && !usesDefStat)
            modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(1.5));
    }

    // field abilities
    if (IsAbilityOnField(ABILITY_SWORD_OF_RUIN) && !HasActiveAbility(battlerDef, ABILITY_SWORD_OF_RUIN) && usesDefStat)
        modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(0.75));

    if (IsAbilityOnField(ABILITY_BEADS_OF_RUIN) && !HasActiveAbility(battlerDef, ABILITY_BEADS_OF_RUIN) && !usesDefStat)
        modifier = uq4_12_multiply_half_down(modifier, UQ_4_12(0.75));
...
{
    bool32 hasAdaptability = HasActiveAbility(ctx->battlerAtk, ABILITY_ADAPTABILITY);
    ...
    if (ctx->moveType == TYPE_MYSTERY)
        ...
        return hasAdaptability ? UQ_4_12(2.0) : UQ_4_12(1.5);
...
static inline uq4_12_t GetBurnOrFrostBiteModifier(struct DamageContext *ctx)
{
    ...
    if (gBattleMons[ctx->battlerAtk].status1 & STATUS1_BURN
        ...
        && !HasActiveAbility(ctx->battlerAtk, ABILITY_GUTS))
        return UQ_4_12(0.5);
...
struct AttackerDamageAbilityCtx
{
    struct DamageContext *ctx;
    uq4_12_t *modifier;
};

static struct AttackerDamageAbilityCtx *sAttackerDamageAbilityCtx;

static bool32 ApplyAttackerDamageAbility(u16 ability)
{
    struct AttackerDamageAbilityCtx *ctx = sAttackerDamageAbilityCtx;
    struct DamageContext *dmgCtx = ctx->ctx;

    switch (ability)
    {
    case ABILITY_NEUROFORCE:
        if (dmgCtx->typeEffectivenessModifier >= UQ_4_12(2.0))
            *ctx->modifier = uq4_12_multiply(*ctx->modifier, UQ_4_12(1.25));
        break;
    case ABILITY_SNIPER:
        if (dmgCtx->isCrit)
    ...
    case ABILITY_TINTED_LENS:
        if (dmgCtx->typeEffectivenessModifier <= UQ_4_12(0.5))
            *ctx->modifier = uq4_12_multiply(*ctx->modifier, UQ_4_12(2.0));
        break;
    }
...
static inline uq4_12_t GetAttackerAbilitiesModifier(struct DamageContext *ctx)
{
    uq4_12_t modifier = UQ_4_12(1.0);
    struct AttackerDamageAbilityCtx abilityCtx = {
        .ctx = ctx,
        .modifier = &modifier,
    };
...
struct DefenderDamageAbilityCtx
{
    struct DamageContext *ctx;
    uq4_12_t *modifier;
};

static struct DefenderDamageAbilityCtx *sDefenderDamageAbilityCtx;

static bool32 ApplyDefenderDamageAbility(u16 ability)
{
    struct DefenderDamageAbilityCtx *ctx = sDefenderDamageAbilityCtx;
    struct DamageContext *dmgCtx = ctx->ctx;
    ...
    case ABILITY_SHADOW_SHIELD:
        if (IsBattlerAtMaxHp(dmgCtx->battlerDef))
        {
    ...
    case ABILITY_PRISM_ARMOR:
        ...
        break;
    case ABILITY_FLUFFY:
        if (dmgCtx->moveType == TYPE_FIRE && !IsMoveMakingContact(dmgCtx->battlerAtk, dmgCtx->battlerDef, ABILITY_NONE, dmgCtx->holdEffectAtk, dmgCtx->move))
        {
    ...
    case ABILITY_ICE_SCALES:
        if (IsBattleMoveSpecial(dmgCtx->move))
        {
            modifier =  UQ_4_12(0.5);
            recordAbility = TRUE;
        }
...
{
    uq4_12_t modifier = UQ_4_12(1.0);
    struct DefenderDamageAbilityCtx abilityCtx = {
        .ctx = ctx,
        .modifier = &modifier,
    };
...
static inline uq4_12_t GetDefenderPartnerAbilitiesModifier(u32 battlerPartnerDef)
{
    ...
    if (HasActiveAbility(battlerPartnerDef, ABILITY_FRIEND_GUARD))
        return UQ_4_12(0.75);
    return UQ_4_12(1.0);
}
...
static inline uq4_12_t GetDefenderItemsModifier(struct DamageContext *ctx)
{
    ...
    case HOLD_EFFECT_RESIST_BERRY:
        ...
        if (ctx->moveType == GetBattlerHoldEffectParam(ctx->battlerDef) && (ctx->moveType == TYPE_NORMAL || ctx->typeEffectivenessModifier >= UQ_4_12(2.0)))
        {
            ...
            return HasActiveAbility(ctx->battlerDef, ABILITY_RIPEN) ? UQ_4_12(0.25) : UQ_4_12(0.5);
        }
        break;
    default:
        break;
...
static inline uq4_12_t GetOtherModifiers(struct DamageContext *ctx)
{
    ...
    {
        DAMAGE_MULTIPLY_MODIFIER(GetAttackerAbilitiesModifier(ctx));
...
static inline void MulByTypeEffectiveness(struct DamageContext *ctx, uq4_12_t *modifier, u32 defType)
{
    ...
    else if ((ctx->moveType == TYPE_FIGHTING || ctx->moveType == TYPE_NORMAL) && defType == TYPE_GHOST && gBattleMons[ctx->battlerDef].volatiles.foresight && mod == UQ_4_12(0.0))
    {
        ...
        && mod == UQ_4_12(0.0))
    {
        bool32 hasScrappy = HasActiveAbility(ctx->battlerAtk, ABILITY_SCRAPPY);
        bool32 hasMindsEye = HasActiveAbility(ctx->battlerAtk, ABILITY_MINDS_EYE);
        if (hasScrappy || hasMindsEye)
        {
            mod = UQ_4_12(1.0);
            if (ctx->updateFlags)
                RecordAbilityBattle(ctx->battlerAtk, hasScrappy ? ABILITY_SCRAPPY : ABILITY_MINDS_EYE);
        }
    }
...
static inline uq4_12_t CalcTypeEffectivenessMultiplierInternal(struct DamageContext *ctx, uq4_12_t modifier)
{
    ...
    {
        modifier = UQ_4_12(0.0);
        if (ctx->updateFlags && HasActiveAbility(ctx->battlerDef, ABILITY_LEVITATE))
        {
...
bool32 CanFling(u32 battler)
{
    ...
    if (item == ITEM_NONE
      || (B_KLUTZ_FLING_INTERACTION >= GEN_5 && HasActiveAbility(battler, ABILITY_KLUTZ))
...
{
    return HasActiveAbility(battler, ABILITY_SHEER_FORCE) && MoveIsAffectedBySheerForce(move);
}
...
static bool32 CanBeInfinitelyConfused(u32 battler)
{
    if  (HasActiveAbility(battler, ABILITY_OWN_TEMPO)
      || IsBattlerTerrainAffected(battler, STATUS_FIELD_MISTY_TERRAIN)
...
static inline bool32 DoesBattlerHaveAbilityImmunity(u32 battlerAtk, u32 battlerDef, u32 moveType)
{
    ...
    return CanAbilityBlockMove(battlerAtk, battlerDef, GetBattlerAbility(battlerAtk), abilityDef, gCurrentMove, CHECK_TRIGGER)
        || GetBattlerMoveAbsorbingAbility(battlerAtk, battlerDef, abilityDef, gCurrentMove, moveType) != ABILITY_NONE;
}
...
bool32 IsMoveEffectBlockedByTarget(u32 battler, u32 ability)
{
    if (ability == ABILITY_SHIELD_DUST || HasActiveAbility(battler, ABILITY_SHIELD_DUST))
    {
        RecordAbilityBattle(battler, ABILITY_SHIELD_DUST);
        return TRUE;
    }
    else if (GetBattlerHoldEffect(battler, TRUE) == HOLD_EFFECT_COVERT_CLOAK)
    {
...
void UpdateStallMons(void)
{
    ...
    if (!IsDoubleBattle() || gMovesInfo[gCurrentMove].target == MOVE_TARGET_SELECTED)
    {
        ...
        if (GetBattlerMoveAbsorbingAbility(gBattlerAttacker, gBattlerTarget, abilityDef, gCurrentMove, moveType) != ABILITY_NONE)
        {
...
bool32 CanMoveSkipAccuracyCalc(u32 battlerAtk, u32 battlerDef, u32 abilityAtk, u32 abilityDef, u32 move, enum FunctionCallOption option)
{
    ...
    u32 abilityBattler = battlerAtk;
    enum BattleMoveEffects moveEffect = GetMoveEffect(move);
    u32 nonVolatileStatus = GetMoveNonVolatileStatus(move);
    bool32 attackerNoGuard = option == AI_CHECK ? abilityAtk == ABILITY_NO_GUARD : HasActiveAbility(battlerAtk, ABILITY_NO_GUARD);
    bool32 defenderNoGuard = option == AI_CHECK ? abilityDef == ABILITY_NO_GUARD : HasActiveAbility(battlerDef, ABILITY_NO_GUARD);
...
static u32 GetTotalAccuracyInternal(u32 battlerAtk, u32 battlerDef, u32 move, u32 atkAbility, u32 defAbility, u32 atkHoldEffect, u32 defHoldEffect, bool32 useActiveAbilities)
{
    u32 calc, moveAcc;
    ...
    if ((useActiveAbilities
      && (HasActiveAbility(battlerAtk, ABILITY_UNAWARE)
       || HasActiveAbility(battlerAtk, ABILITY_KEEN_EYE)
       || HasActiveAbility(battlerAtk, ABILITY_MINDS_EYE)
       || (GetGenConfig(GEN_ILLUMINATE_EFFECT) >= GEN_9 && HasActiveAbility(battlerAtk, ABILITY_ILLUMINATE))))
     || (!useActiveAbilities
    ...
    if (MoveIgnoresDefenseEvasionStages(move))
        evasionStage = DEFAULT_STAT_STAGE;
    if ((useActiveAbilities && HasActiveAbility(battlerDef, ABILITY_UNAWARE))
     || (!useActiveAbilities && defAbility == ABILITY_UNAWARE))
    ...
    if (IsBattlerWeatherAffected(battlerDef, B_WEATHER_SUN) && MoveHas50AccuracyInSun(move))
        moveAcc = 50;
    // Check Wonder Skin.
    if (((useActiveAbilities && HasActiveAbility(battlerDef, ABILITY_WONDER_SKIN))
      || (!useActiveAbilities && defAbility == ABILITY_WONDER_SKIN))
    ...
    if (useActiveAbilities)
    {
        if (HasActiveAbility(battlerAtk, ABILITY_COMPOUND_EYES))
            calc = (calc * 130) / 100; // 1.3 compound eyes boost
        if (HasActiveAbility(battlerAtk, ABILITY_VICTORY_STAR))
            calc = (calc * 110) / 100; // 1.1 victory star boost
        if (HasActiveAbility(battlerAtk, ABILITY_HUSTLE) && IsBattleMovePhysical(move))
    ...
    if (useActiveAbilities)
    {
        if (HasActiveAbility(battlerDef, ABILITY_SAND_VEIL) && HasWeatherEffect() && gBattleWeather & B_WEATHER_SANDSTORM)
            calc = (calc * 80) / 100; // 1.2 sand veil loss
        if (HasActiveAbility(battlerDef, ABILITY_SNOW_CLOAK) && HasWeatherEffect() && (gBattleWeather & (B_WEATHER_HAIL | B_WEATHER_SNOW)))
            calc = (calc * 80) / 100; // 1.2 snow cloak loss
        if (HasActiveAbility(battlerDef, ABILITY_TANGLED_FEET) && gBattleMons[battlerDef].volatiles.confusionTurns)
    ...
    if (IsBattlerAlive(atkAlly)
     && HasActiveAbility(atkAlly, ABILITY_VICTORY_STAR)
     && (!useActiveAbilities || !HasActiveAbility(battlerAtk, ABILITY_VICTORY_STAR)))
    ...
    {
        if ((useActiveAbilities && HasActiveAbility(battlerAtk, ABILITY_RIPEN))
         || (!useActiveAbilities && atkAbility == ABILITY_RIPEN))
...
{
    return GetTotalAccuracyInternal(battlerAtk, battlerDef, move, atkAbility, defAbility, atkHoldEffect, defHoldEffect, FALSE);
}

u32 GetTotalAccuracyActive(u32 battlerAtk, u32 battlerDef, u32 move, u32 atkHoldEffect, u32 defHoldEffect)
{
    return GetTotalAccuracyInternal(battlerAtk, battlerDef, move, ABILITY_NONE, ABILITY_NONE, atkHoldEffect, defHoldEffect, TRUE);
}
